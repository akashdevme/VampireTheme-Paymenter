...
<label for="{{ $id }}" class="flex items-center {{ $disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer' }}">
    <div class="relative">
        <input
            ...
            {{ $disabled ? 'disabled' : '' }}
        >
        <div class="w-11 h-6 bg-neutral rounded-full peer-checked:bg-primary peer-checked:shadow-[0_0_14px_hsl(var(--color-primary)/0.55)] transition-all duration-300 ease-in-out"></div>
